...
**Migrated this session:** Replace `python-jose` with `PyJWT` in backend dependencies (`requirements.txt`) and update all `jose` imports and method calls (`jwt.encode`, `jwt.decode`, `jwt.get_unverified_claims`) across the codebase.
