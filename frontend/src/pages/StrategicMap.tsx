...
const StrategicMap: React.FC = () => {
  ...
  const onEachFeature = (feature: any, layer: any) => {
    ...
    if (d) {
      // Oracle: Predictive Prefetch on hover
      layer.on('mouseover', () => {
        queryClient.prefetchQuery({
          queryKey: ['district-detail', districtId, 'dengue'],
          queryFn: async () => {
            const url = `${import.meta.env.VITE_API_URL}/districts/${districtId}?disease=dengue`;
            const response = await axios.get(url);
            return response.data;
          },
