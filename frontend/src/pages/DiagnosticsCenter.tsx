...
const DiagnosticsCenter: React.FC = () => {
  ...
  // Oracle 🛸 - Behavioral Prefetch
  // When a user runs a tactical diagnosis, there is a high probability they will
  // download the PDF report immediately after. Instead of making them wait for the PDF
  // to be generated upon clicking the download button, we prefetch it silently in the background
  // and have the Blob URL ready to go.
  useEffect(() => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      // Fallback: if prefetch didn't finish or failed, fetch on demand
      if (!url) {
        const response = await apiClient.post(
          '/clinical/report',
          [prediction],
