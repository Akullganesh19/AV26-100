...
const DiagnosticsCenter: React.FC = () => {
  ...
  // Predictive Report Generation State
  const [prefetchedReportUrl, setPrefetchedReportUrl] = useState<string | null>(null);

  // Oracle Prediction: Anticipate that users download reports after a diagnosis.
  // Pre-generate and fetch the PDF report in the background immediately after a prediction is made.
  useEffect(() => {
    let isActive = true;
    ...
    const prefetchReport = async () => {
      ...
      try {
        ...
        // Silently fail prefetching, the manual download will fall back to fetching on demand
      }
    };
  ...
  const handleDiagnose = async (formData: any) => {
    setLoading(true);
    setPrediction(null);
    setPrefetchedReportUrl(null); // Reset prefetched url
    try {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      // If we created a new URL just for this fallback, clean it up
      if (!prefetchedReportUrl && url) {
        setTimeout(() => window.URL.revokeObjectURL(url as string), 100);
      }
