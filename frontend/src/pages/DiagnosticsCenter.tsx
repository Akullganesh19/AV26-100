        ...
        <div className="flex flex-wrap gap-2">
          {vocalMetrics.map((v, i) => (
            <div key={i} className="flex flex-col gap-1 w-20">
              ...
              {/* 🎨 UX Concern: Vocal metric input lacks accessible name */}
              <input 
                type="number" 
                step="0.0001"
...
const Input = ({ label, type, value, onChange, step }: any) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {/* 🎨 UX Concern: Custom Input helper missing htmlFor and id */}
...
const Select = ({ label, value, options, onChange }: any) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      {/* 🎨 UX Concern: Custom Select helper missing htmlFor and id */}
      <label htmlFor={id} className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{label}</label>
      <select
        id={id}
        value={value}
