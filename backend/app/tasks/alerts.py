...
async def send_alert_notification(alert_id: str, district_name: str, disease: str, risk_score: float, email: str = None):
    """
    ...
    try:
        ...
        target = f" to {email}" if email else ""
        logger.info(f"CRITICAL ALERT: Outbreak risk detected in {district_name} ({disease}). Score: {risk_score}{target}")
