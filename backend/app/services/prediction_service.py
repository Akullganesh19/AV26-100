...
class PredictionService:
    ...
    async def predict_batch(
        ...
        Queries the database first for existing cached predictions.
        Uses a semaphore to prevent overwhelming the database connection pool or CPU.
        """
        overrides = overrides or {}

        result_map = {}
        missing_district_ids = district_ids

        # If no overrides, try to fetch from cache first
        if not overrides:
            ...
            missing_district_ids = [d_id for d_id in district_ids if d_id not in result_map]
