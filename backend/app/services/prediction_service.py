...
class PredictionService:
    ...
    async def predict_batch(
        ...
        """
        # Step 1: Bulk query existing predictions to avoid re-computation
        ...
        # Step 2: Map cached predictions
        cached_results = {}
        for p in existing_predictions:
            cached_results[p.district_id] = PredictionResponse(
                prediction_id=p.id,
        ...
        # Step 4: Compute missing predictions concurrently
        ...
        # Map computed results
        for r in computed_results:
            if r is not None:
                cached_results[r.district_id] = r

        # Step 5: Reconstruct the final list preserving the original order
        final_results = [cached_results[d_id] for d_id in district_ids if d_id in cached_results]
