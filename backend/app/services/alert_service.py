...
class AlertService:
        ...
        try:
            ...
            if count >= settings.CLINICAL_CLUSTER_THRESHOLD:
                ...
                if not existing.scalar():
                    ...
                    event_bus.publish("alert.triggered", district_id, disease, 0.88)
    ...
    async def evaluate_autonomous_outbreak(db: AsyncSession, prediction: Prediction):
        ...
        if prediction.risk_score * 100 > settings.ALERT_THRESHOLD_DEFAULT:
            ...
            db.add(new_alert)
            await db.commit()
            event_bus.publish("alert.triggered", str(prediction.district_id), prediction.disease, float(prediction.risk_score))
