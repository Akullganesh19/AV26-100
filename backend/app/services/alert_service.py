...
class AlertService:
    @staticmethod
    async def evaluate_clinical_cluster(
        db: AsyncSession, district_id: str, disease: str
    ):
        """
        ...
        try:
            ...
            if count >= settings.CLINICAL_CLUSTER_THRESHOLD:
                ...
                if not existing.scalar():
                    ...
                    logger.info(
                        f"TACTICAL ALERT: Clinical cluster detected in {district_id} ({disease})"
                    )

                    # SYNAPSE: Publish event to allow cross-system intelligence (like targeted user notifications)
                    await event_bus.publish(
                        "alert.created",
                        {
                            "alert_id": str(new_alert.id),
                            "district_id": str(new_alert.district_id),
                            "disease": new_alert.disease,
    ...
    async def evaluate_autonomous_outbreak(db: AsyncSession, prediction: Prediction):
        ...
        if prediction.risk_score * 100 > settings.ALERT_THRESHOLD_DEFAULT:
            ...
            # SYNAPSE: Publish event for cross-system intelligence
            await event_bus.publish(
                "alert.created",
                {
                    "alert_id": str(new_alert.id),
