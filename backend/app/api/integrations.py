...
class IntegrationService:
    ...
    @with_retry(max_attempts=3, base_delay=1.0, fallback_value=None)
    @with_circuit_breaker(
        failure_threshold=3, recovery_timeout=60.0, fallback_value=None
    )
    async def upload_report_to_cloudinary(
        self, file_bytes: bytes, district_id: str
    ):
