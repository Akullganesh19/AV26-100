...
class IntegrationService:
    ...
    @with_retry(max_attempts=3, idempotent_required=True)
    ...
    @with_retry(max_attempts=3, idempotent_required=False) # Emails aren't strictly idempotent without advanced setup, but retries are typically safe for alerts
    ...
    @with_circuit_breaker(failure_threshold=3, recovery_timeout=30.0)
    @with_retry(max_attempts=3, idempotent_required=False)
