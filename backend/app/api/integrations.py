...
class IntegrationService:
    ...
    async def sync_district_to_algolia(self, district_data: dict):
        ...
        # Offload sync I/O to a separate thread with retry wrapper
        try:
            ...
            # Graceful degradation - it's a sync to a search engine, we shouldn't fail the whole request
            pass
    ...
    async def upload_report_to_cloudinary(self, file_bytes: bytes, district_id: str):
        ...
        try:
            upload_result = await with_retry(asyncio.to_thread, _upload)
            return upload_result.get("secure_url")
        except Exception as e:
            logger.error(f"Failed to upload report to Cloudinary for district {district_id}: {e}")
