...
async def diagnose_heart(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Heart diagnosis failed", exc_info=True)
        await log_prediction(db, current_user.id, "clinical/heart", data.dict(), status="FAIL", error=str(e), district_id=data.district_id)
        raise HTTPException(status_code=500, detail="Clinical diagnosis failed due to an internal error")
...
async def diagnose_diabetes(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Diabetes diagnosis failed", exc_info=True)
...
async def diagnose_parkinsons(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Parkinsons diagnosis failed", exc_info=True)
