...
async def diagnose_heart(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Clinical heart prediction failed: %s", str(e), exc_info=True)
...
async def diagnose_diabetes(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Clinical diabetes prediction failed: %s", str(e), exc_info=True)
...
async def diagnose_parkinsons(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("Clinical parkinsons prediction failed: %s", str(e), exc_info=True)
...
async def generate_screening_report(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("PDF generation failed: %s", str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="Internal Server Error")
