...
async def diagnose_heart(
    ...
    try:
        ...
        # 🛡️ SENTINEL: Log type(e).__name__ or raw error in DB, but return generic HTTP 500
        await log_prediction(db, current_user.id, "clinical/heart", data.dict(), status="FAIL", error=str(e), district_id=data.district_id)
        raise HTTPException(status_code=500, detail="Tactical screening failed")
...
async def generate_screening_report(
    ...
    try:
        ...
        # 🛡️ SENTINEL: Return generic error to avoid leaking path/internal errors
