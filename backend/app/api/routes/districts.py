...
async def get_district_detail(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error(f"Failed to retrieve detail for district {district_id}", exc_info=True)
        return {
            "id": str(district.id),
            "name": district.name,
            "state": district.state,
            "risk_score": 0,
            "risk_tier": "unknown",
            "error": "Failed to retrieve district detail due to an internal error"
        }
