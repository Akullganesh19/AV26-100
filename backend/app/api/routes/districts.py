...
async def get_district_stats(
    ...
    # Total Districts and Population (Combined Query)
    q_districts = await db.execute(select(func.count(District.id), func.sum(District.population)))
    total, pop = q_districts.one_or_none() or (0, 0)
    total = total or 0
    pop = pop or 0
