...
async def list_districts(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve jurisdiction matrix. Please try again later.")
...
async def get_district_detail(
    ...
    try:
        ...
        logger.exception(f"Failed to retrieve district details for {district_id}")
        return {
            "id": str(district.id),
            "name": district.name,
            "state": district.state,
