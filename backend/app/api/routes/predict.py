...
async def create_prediction(
    ...
    try:
        ...
        logger.warning(f"Insufficient history for prediction: {e}")
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail={"code": "INSUFFICIENT_HISTORY", "message": "Not enough historical data for prediction"},
        )
    except Exception as e:
        logger.error(f"Inference engine failure: {e}", exc_info=True)
        raise HTTPException(
