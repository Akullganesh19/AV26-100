...
class FeatureBuilder:
    ...
    async def build(
        ...
        if not df.empty:
            # Optimization: Use argmax() which is O(N) instead of sort_values() which is O(N log N)
            # argmax() is safer than idxmax() as it uses positional indexing, avoiding issues with non-unique indices
            return pd.DataFrame([df.iloc[df["week_start_date"].argmax()]])
        return pd.DataFrame()
