...
class FeatureBuilder:
    ...
    async def build(
        ...
        if not df.empty:
            # ⚡ Bolt Optimization:
            # The SQL query already orders by week_start_date DESC.
            # Pandas boolean filtering preserves this order, so we avoid
            # an O(N log N) df.sort_values() and use an O(1) df.iloc[0].
